...
function filter_children(parrentEelement, selector, attr, value){    
    Array.from(parrentEelement.children).forEach(child => {
        const withinChildElement = child.querySelector(selector);
    if (withinChildElement && withinChildElement[attr] != value) {
        child.remove();
    }
});
...
function filter_out_children(parrentEelement, selector, attr, value){    
    Array.from(parrentEelement.children).forEach(child => {
        const withinChildElement = child.querySelector(selector);
    if (withinChildElement && withinChildElement[attr] == value) {
        child.remove();
